...
const STRING_FIELDS = [
  "orderId", "ticketId", "customerName", "customerEmail", "country",
  "details", "suggestion",
];
const ENUMS = {
  theme: [
    "product", "packaging", "subscription", "shipping", "pricing",
    "marketing", "loyalty", "tech", "service", "other",
  ],
  relatedTeam: ["Product", "Marketing", "Ops/Logistics", "Tech", "CX", "Other"],
};
...
export async function PATCH(req, { params }) {
  ...
  for (const [field, allowed] of Object.entries(ENUMS)) {
    if (body[field] !== undefined) {
      // relatedTeam is nullable — allow clearing it
      if (field === "relatedTeam" && (body[field] === null || body[field] === "")) {
        data[field] = null;
        continue;
      }
  ...
  try {
    const row = await db.feedback.update({ where: { id }, data });
