...
/**
 * Parse the im8-macros-v2 markdown file into structured macro records.
 *
 * Expected structure per macro:
 *   ### IM8: SlugName
...
export function parseMacrosMarkdown(md) {
  ...
  while (i < lines.length) {
    ...
    // Macro header: "### IM8: Slug"
    const slugMatch = line.match(/^###\s+IM8:\s+(.+?)\s*$/);
    if (slugMatch && currentCategory) {
      ...
      if (notes) {
        ...
        else if (n.includes("escalate to cherie")) escalationRule = "head_of_cx";
      }
