...
// A busy week can produce 10K+ tickets. 250 × 100 = 25K is generous
// headroom; pagination still stops early when we cross the date window
// naturally. Lower MAX_PAGES if the brand's volume is much smaller.
const MAX_PAGES = 250;

// If this Gorgias account is shared across multiple brands, set
// BRAND_TICKET_TAG to the tag that marks this brand's tickets (used by
// filterBrandTickets / topNonBrandTags). Leave as-is for a single-brand
// account.
const BRAND_TICKET_TAG = process.env.BRAND_TICKET_TAG || "[BRAND_TAG]";
...
export function filterBrandTickets(tickets) {
  return tickets.filter((t) =>
    Array.isArray(t.tags) && t.tags.some((tag) => tag?.name === BRAND_TICKET_TAG)
  );
}
...
export function topNonBrandTags(tickets, n = 5) {
  ...
  for (const t of tickets) {
    ...
    for (const tag of tags) {
      const name = tag?.name;
      if (!name) continue;
      if (name === BRAND_TICKET_TAG) continue;
...
export async function fetchCustomerFirstMessagesBulk(fromIso, toIso, ticketIds, maxChars = 280) {
  ...
  let cursor = null;
  let pages = 0;
  // Big ranges (e.g. a busy "Today" with 1000-2000+ tickets) can have
  // 30k+ messages. 350 pages × 100 = 35k messages, enough to cover most
  // days. With 50ms inter-page delay that's ~17.5 sec worst case before Claude.
  const MAX_PAGES_LOCAL = 350;
