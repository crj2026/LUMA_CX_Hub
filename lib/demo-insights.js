...
import { seededSeries } from "./chart-utils.js";
...
// ─── Headline-stat trends ────────────────────────────────────────────────────
// 30-day sparkline series + delta chip vs the previous period for every
// headline stat. `good` says whether this movement is a win (drives the
// muted green/red of the chip); series endpoints match the headline
// numbers so the line always lands on the stat it sits under.
function t(key, start, end, delta, good, volatility = 0.05) {
  return { series: seededSeries(key, 30, start, end, volatility), delta, good };
}

export const DEMO_TREND = {
  tickets:        t("tickets", 790, 847, "+7%", false),
  open:           t("open", 47, 41, "-13%", true, 0.12),
  closed:         t("closed", 690, 724, "+5%", true),
  csat:           t("csat", 4.4, 4.6, "+0.2", true, 0.02),
  resolution:     t("resolution", 2250, 2040, "-9%", true, 0.06),
  mpt:            t("mpt", 3.4, 3.2, "-6%", true, 0.04),
  orders:         t("orders", 2640, 2847, "+8%", true),
  refunds:        t("refunds", 65, 60, "-8%", true, 0.1),
  refundsTotal:   t("refundsTotal", 5200, 4800, "-8%", true, 0.1),
  refundRate:     t("refundRate", 0.024, 0.021, "-0.3 pts", true, 0.08),
  cancelRate:     t("cancelRate", 0.009, 0.008, "-0.1 pts", true, 0.1),
  revenue:        t("revenue", 216000, 233454, "+8%", true),
  aov:            t("aov", 80, 82, "+2%", true, 0.02),
  activeSubs:     t("activeSubs", 8268, 8432, "+2%", true, 0.01),
  churnRate:      t("churnRate", 0.02, 0.018, "-0.2 pts", true, 0.08),
  cancelledSubs:  t("cancelledSubs", 156, 148, "-5%", true, 0.1),
  createdSubs:    t("createdSubs", 281, 312, "+11%", true, 0.08),
  failedPayments: t("failedPayments", 29, 34, "+18%", false, 0.12),
  mrr:            t("mrr", 728000, 750448, "+3%", true, 0.01),
  saveRate:       t("saveRate", 0.37, 0.43, "+6 pts", true, 0.06),
};
